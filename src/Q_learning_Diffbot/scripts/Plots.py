...
# DATA_PATH = '/home/duy/catkin_ws/src/autonomous-driving-turtlebot-with-reinforcement-learning/Data'
...
def plot_learning(log_file_dir):
    ...
    episodes_100 = np.arange(0, len(reward_per_episode)+100, 100)
    ...
    # for i in range(len(episodes_100)):
    #     accumulated_reward = np.append(accumulated_reward, np.sum(reward_per_episode[0:100*(i+1)]))
    ...
    plt.figure(1)
    plt.subplot(223)
    plt.plot(T_per_episode, color='darkviolet')
    plt.xlabel('Episode')
    plt.ylabel('T')
    ...
    plt.figure(1)
    plt.subplot(224)
    plt.plot(reward_max_per_episode, label = 'max rewards', color='teal')
    plt.plot(reward_avg_per_episode, label = 'avg rewards', color='darkkhaki' )
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    ...
    plt.figure(2)
    plt.plot(episodes_10, accumulated_reward, color='teal')
    plt.xlabel('Episode')
    plt.ylabel('Accumulated reward')
    plt.title('Accumulated reward per 10 episodes')
    ...
    plt.figure(3)
    plt.plot(episodes_10, av_steps_per_10_episodes, color='darkslateblue')
    plt.xlabel('Episode')
    plt.ylabel('Average steps')
    plt.title('Average steps per 10 episode')
...
def plot_Q_table(log_file_dir):
    ...
    ax.bar(states, Q_table_norm[:,0], width, color = 'indianred')
    ax.bar(states, Q_table_norm[:,1], width, bottom = Q_table_norm[:,0], color = 'mediumturquoise')
    ax.bar(states, Q_table_norm[:,2], width, bottom = Q_table_norm[:,0] + Q_table_norm[:,1], color = 'darkkhaki')
...
# plot_Q_table(DATA_PATH + '/Epsilon_Log_21-5')
# plot_Q_table(DATA_PATH + '/rl_test_50')
plot_learning(DATA_PATH + '/new_state_def_eps_71022')
