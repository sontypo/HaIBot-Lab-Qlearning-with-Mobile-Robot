...
#Viet thu vien function Q-learning algorithm
...
CONST_LINEAR_SPEED_FORWARD = 0.35 #0.15 , 0.5, 0.35
CONST_ANGULAR_SPEED_FORWARD = 0.0
CONST_LINEAR_SPEED_TURN = 0.15 #0.1 , 0.15 , 0.2
CONST_ANGULAR_SPEED_TURN = 0.5 # 0.5 , 1.0, 0.65, 0.6
VERY_LINEAR_SPEED_TURN = 0.0
VERY_ANGULAR_SPEED_TURN = 0.6

CONST_LINEAR_SPEED_45 = 0.07
CONST_ANGULAR_SPEED_45 = 0.07
CONST_LINEAR_SPEED_AROUND = 0.0
CONST_ANGULAR_SPEED_AROUND = 0.4
...
# Go forward command at 90 degrees
def robotGoForward(velPub):
    velMsg = createVelMsg(CONST_LINEAR_SPEED_FORWARD,CONST_ANGULAR_SPEED_FORWARD)
    velPub.publish(velMsg)

# Go with 45 degrees on the left
def robot45OnLeft(velPub):
    velMsg = createVelMsg(CONST_LINEAR_SPEED_45,CONST_ANGULAR_SPEED_45)
    velPub.publish(velMsg)

# Go with 45 degrees on the right
def robot45OnRight(velPub):
    velMsg = createVelMsg(CONST_LINEAR_SPEED_45,-CONST_ANGULAR_SPEED_45)
    velPub.publish(velMsg)
...
# 100% left turned command
def robotTrueLeftTurned(velPub):
    velMsg = createVelMsg(VERY_LINEAR_SPEED_TURN,VERY_ANGULAR_SPEED_TURN)
    velPub.publish(velMsg)
    
# 100% right turned command
def robotTrueRightTurned(velPub):
    velMsg = createVelMsg(VERY_LINEAR_SPEED_TURN,-VERY_ANGULAR_SPEED_TURN)
    velPub.publish(velMsg)
...
# Round command
def robotGoAround(velPub):
    velMsg = createVelMsg(CONST_LINEAR_SPEED_AROUND,CONST_ANGULAR_SPEED_AROUND)
    velPub.publish(velMsg)

def robotTurnLeftRound(velPub):
    velMsg = createVelMsg(CONST_LINEAR_SPEED_AROUND,CONST_ANGULAR_SPEED_AROUND)
    velPub.publish(velMsg)

def robotTurnRightRound(velPub):
    velMsg = createVelMsg(CONST_LINEAR_SPEED_AROUND,-CONST_ANGULAR_SPEED_AROUND)
    velPub.publish(velMsg)
...
def robotSetPos(setPosPub, x, y, theta):
    ...
    checkpoint.model_name = 'diff_robot'
...
def robotDoAction(velPub, action):
    ...
    if action == 0:
        ...
        robotTrueLeftTurned(velPub)
    elif action == 4:
        robotTrueRightTurned(velPub)
    else:
